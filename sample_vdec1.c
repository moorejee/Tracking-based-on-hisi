...
// char** sample_yuv_dump(PIXEL_FORMAT_E , HI_U32 , HI_U32 , HI_VOID* , HI_VOID*);
    ...
    while (pstSendParam->bRun)
    {
        ...
        ******************************************/
        ...
        if(pstSendParam->VdChn == 0 && s32BlockMode == HI_IO_NOBLOCK)
        {
			...
			Imagelen0 = pstFrameInfo->stVFrame.u32Stride[0] * pstFrameInfo->stVFrame.u32Height; //原始数据长度，数据存储的宽度为stride>=width.
			//stride是为了将数据长度向上按16位或是64位补齐
			Imagelen = pstFrameInfo->stVFrame.u32Width * pstFrameInfo->stVFrame.u32Height;//处理后的真实数据的长度．
			...
			s32Ret = HI_MPI_VDEC_ReleaseImage(pstSendParam->VdChn, pstFrameInfo); //release与get应对应
			if (s32Ret != HI_SUCCESS)
				printf("release image error!\n");
			fcount++;
        }
...
void SAMPLE_VDEC_HandleSig(HI_S32 signo)
{
    ...
    if (SIGINT == signo || SIGTSTP == signo)
    {
        ...
        for (i=0; i<gs_s32Cnt; i++)
        {
            ...
            {
                gs_SendParam[i].bRun = HI_FALSE;//终端程序运行
